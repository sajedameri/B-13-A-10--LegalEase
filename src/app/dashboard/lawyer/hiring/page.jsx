    ...
    <div className="w-full max-w-5xl mx-auto p-6 space-y-6 bg-background">
      ...
      <div className="pb-4 border-b border-divider">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Review client consultation submissions, manage contract approvals, and filter case assignment history.
        </p>
      </div>

      {/* HeroUI v3 Collection Layout Table */}
      <Table variant="primary" className="w-full border border-divider rounded-xl overflow-hidden shadow-3xs">
        <Table.ScrollContainer>
          <Table.Content aria-label="Inbound client retainer legal requests inventory ledger">
            ...
            >
              {(request) => (
                <Table.Row key={request._id} className="border-b border-divider/40 last:border-0 hover:bg-default-50 transition-colors">
                  ...
                  <Table.Cell>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="font-bold text-sm text-foreground tracking-tight">{request.clientName}</span>
                        <span className="text-[10px] text-muted-foreground uppercase tracking-wider font-mono">ID: {request._id.slice(-6)}</span>
                      </div>
                    </div>
                  </Table.Cell>
